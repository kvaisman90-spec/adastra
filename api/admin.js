...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (binRes.ok) {
      ...
      db.invitations = Array.isArray(rec.invitations) ? rec.invitations : [];

      db.settings = Object.assign(
        { wallets: { usdt: '', btc: '', eth: '' } },
        rec.settings || {}
      );

      db.settings.wallets = Object.assign(
        { usdt: '', btc: '', eth: '' },
        (rec.settings && rec.settings.wallets) || {}
      );
    }
